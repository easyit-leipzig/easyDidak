...
select * from mtr_rueckkopplung_datenmaske_values where value in (select concat("%", real_value, "%") from frzk_funktionsklassen_weight_absprachen)

SELECT distinct value, count(id) FROM `mtr_rueckkopplung_datenmaske_values` WHERE value in (select real_value from frzk_funktionsklassen_weight_absprachen) or value like "%absprachen%";
