...
switch( $_POST["command"]) {
    ...
    */
                            $query = "SELECT avg(lernfortschritt) as lernfortschritt, avg(beherrscht_thema) as beherrscht_thema, avg(transferdenken) as transferdenken, avg(basiswissen) as basiswissen, 
                                        avg(lernfortschritt) as lernfortschritt, avg(vorbereitet) as vorbereitet, avg(note) as note FROM `mtr_leistung`";
                            $return -> s = $query;
                            try {
...
;   
                   ...
                   $db_pdo -> query( $q );
                            ...
                            $q = "INSERT INTO `mtr_leistung` (`ue_zuweisung_teilnehmer_id`, teinehmer_id) VALUES ($zwId, $tnId)";
                  $db_pdo -> query( $q );
                             $return -> q = $q;
                           $return -> t = $zwId;
                  $q = "select * from mtr_rueckkopplung_teilnehmer where ue_zuweisung_teilnehmer_id = $zwId";
                            $return -> s = $tnId;
                           print_r( json_encode( $return )); 
    break;
    default:
